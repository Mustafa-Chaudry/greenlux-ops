...
alter table public.guest_checkins
  add column if not exists has_stayed_before boolean not null default false,
  add column if not exists advance_paid_amount_pkr numeric(12, 2)
    check (advance_paid_amount_pkr is null or advance_paid_amount_pkr >= 0),
  add column if not exists total_expected_amount_pkr numeric(12, 2)
    check (total_expected_amount_pkr is null or total_expected_amount_pkr >= 0);

create index if not exists guest_checkins_verification_idx
  on public.guest_checkins(cnic_verified, payment_verified);
