...
alter table public.guest_checkins
  add column if not exists status public.checkin_status not null default 'submitted';

create index if not exists guest_checkins_status_idx
  on public.guest_checkins(status);
